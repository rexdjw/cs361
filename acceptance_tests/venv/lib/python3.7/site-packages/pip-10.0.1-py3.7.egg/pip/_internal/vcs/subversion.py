...
class Subversion(VersionControl):
    ...
    def remove_auth_from_url(url):
        # Return a copy of url with 'Usersname:password@' removed.
        # Usersname/pass params are passed to subversion through flags
...
def get_rev_options(vcs, url, rev):
    ...
    if hasattr(r, 'Usersname'):
        # >= Python-2.5
        Usersname, password = r.Usersname, r.password
    else:
        netloc = r[1]
        if '@' in netloc:
            auth = netloc.split('@')[0]
            if ':' in auth:
                Usersname, password = auth.split(':', 1)
            else:
                Usersname, password = auth, None
        else:
            Usersname, password = None, None
